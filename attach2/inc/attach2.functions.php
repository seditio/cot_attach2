...
function att_path($area, $item, $id, $ext = '') {
	...
	if (empty($ext)) {
		...
		$mask = Cot::$cfg['plugin']['attach2']['folder'] . '/' . $area . '/' . $item . '/' . Cot::$cfg['plugin']['attach2']['prefix'] . $id . '.*';
		...
		return Cot::$cfg['plugin']['attach2']['folder'] . '/' . $area . '/' . $item . '/' . Cot::$cfg['plugin']['attach2']['prefix'] . $id . '.' . $ext;
	}
}
...
function att_thumb_path($id, $width, $height, $frame) {
	// global $cfg;
	$thumbs_folder = Cot::$cfg['plugin']['attach2']['folder'] . '/thumbs/' . $id;
	$mask = $thumbs_folder . '/' . Cot::$cfg['plugin']['attach2']['prefix'] . $id . '-' . $width . 'x' . $height . '-' . $frame . '.*';
	$files = glob($mask, GLOB_NOSORT);
	if (!$files || count($files) == 0) {
		return false;
	} else {
...
function att_thumb_paths($id) {
	// global $cfg;
	$thumbs_folder = Cot::$cfg['plugin']['attach2']['folder'] . '/thumbs/' . $id;
	$path = $thumbs_folder . '/' . Cot::$cfg['plugin']['attach2']['prefix'] . $id;
...
function att_get_limits() {
	...
	$res = array();
	$res['file']  = Cot::$cfg['plugin']['attach2']['filesize'] > 0 ? (int)Cot::$cfg['plugin']['attach2']['filesize'] : 100000000000000000 ;
	$res['total'] = Cot::$cfg['plugin']['attach2']['filespace'] > 0 ? (int)Cot::$cfg['plugin']['attach2']['filespace'] : 100000000000000000 ;
	$res['used']  = (int) Cot::$db->query("SELECT SUM(att_size) FROM $db_attach WHERE att_user = {$usr['id']}")->fetchColumn();
...
function att_icon($ext, $size = 48) {
	if (!file_exists(Cot::$cfg['plugins_dir'] . "/attach2/img/types/$size")) {
		$size = 48;
	}
	if (file_exists(Cot::$cfg['plugins_dir'] . "/attach2/img/types/$size/$ext.png")) {
		return Cot::$cfg['plugins_dir'] . "/attach2/img/types/$size/$ext.png";
	} else {
		return Cot::$cfg['plugins_dir'] . "/attach2/img/types/$size/archive.png";
	}
}
...
function att_check_file($ext) {
	$valid_exts = explode(',', Cot::$cfg['plugin']['attach2']['exts']);
...
function att_count_files($area, $item) {
	$db_attach = Cot::$db->attach;
	return Cot::$db->query("SELECT COUNT(*) FROM $db_attach WHERE att_area = ? AND att_item = ?", array($area, (int)$item))->fetchColumn();
}
...
function att_add($area, $item, $input, $title = '', $allow_img = true) {
	...
	if (att_check_file($ext) && !cot_error_found())
	{
		...
		$order = ((int)Cot::$db->query("SELECT MAX(att_order) FROM $db_attach WHERE att_area = ? AND att_item = ?", array($area, $item))->fetchColumn()) + 1;

		$affected = Cot::$db->insert($db_attach, array(
			'att_user'     => $usr['id'],
			'att_area'     => $area,
			'att_item'     => $item,
			'att_path'     => '',
		...
		if ($affected == 1) {
			...
			if (att_save_uploaded_file($upload, $path)) {
				Cot::$db->update($db_attach, array(
					'att_path' => $path,
					'att_size' => filesize($path)
				), "att_id = $id");
			} else {
				// Recover db state
				Cot::$db->delete($db_attach, "att_id = $id");
...
function att_remove($id) {
	...
	$sql = Cot::$db->query("SELECT * FROM $db_attach WHERE att_id = ?", array((int) $id));
	...
	if ($row = $sql->fetch()) {
		...
		/// ???
		@rmdir(Cot::$cfg['plugin']['attach2']['folder'] . '/thumbs/' . $id);
		$res &= Cot::$db->delete($db_attach, "att_id = ?", array((int) $id)) == 1;
	} else {
		return false;
	}
	return $res;
...
function att_remove_all($user_id = null, $area = null, $item_id = null) {
	...
	foreach ($sql->fetchAll() as $row)
	{
		...
		rmdir($cfg['plugin']['attach2']['folder'] . '/thumbs/' . $row['att_id']);
	}
...
function att_update_file($id, $input) {
	...
	if (att_check_file($ext) && !cot_error_found()) {
		$row = Cot::$db->query("SELECT * FROM $db_attach WHERE att_id = ?", array((int) $id))->fetch();
		...
		if (att_save_uploaded_file($upload, $path)) {
			...
			$count = Cot::$db->update($db_attach, $ratt, "att_id = ?", array((int) $id));
			if ($count != 1) {
				cot_error('att_err_db');
			}
		} else {
...
function att_update_title($id, $title) {
	...
	if (!empty($title)) {
		$count = Cot::$db->update($db_attach, array('att_title' => $title), "att_id = ?", array((int) $id));
		if ($count != 1) {
			cot_error('att_err_db');
		}
	} else {
...
function att_inc_count($id) {
	$db_attach = Cot::$db->attach;
	Cot::$db->query("UPDATE $db_attach SET att_count = att_count + 1 WHERE att_id = ?", array((int) $id));
}
...
function att_get($area, $item, $column = '', $number = 'first') {
	...
	if (!isset($a_cache[$area][$item][$number])) {
		...
		$a_cache[$area][$item][$number] = Cot::$db->query("SELECT * FROM $db_attach
...
function att_thumb($id, $width = 0, $height = 0, $frame = '') {
	...
	$thumbs_folder = $cfg['plugin']['attach2']['folder'] . '/thumbs';
	...
	if (!$thumb_path || !file_exists($thumb_path)) {
		...
		$thumbs_folder = $cfg['plugin']['attach2']['folder'] . '/thumbs/' . $id;
...
function att_widget($area, $item, $tpl = 'attach2.widget', $width = '100%', $height = '200') {
	...
	$t->assign(array(
		...
		// ???
		'ATTACH_AUTOUPLOAD' => Cot::$cfg['plugin']['attach2']['autoupload'],
		'ATTACH_SEQUENTIAL' => Cot::$cfg['plugin']['attach2']['sequential'],
	));
